...
class QuestionSchema(BaseModel):
    ...
    evaluation_targets: list[str] = Field(
        description=(
            "이 질문이 실제로 파악하려는 포인트를 짧은 한국어 구절 1~3개로 작성. "
            "예: ['DataLoader 배치 전략 이해', '체크포인트 복원 절차 경험', 'OOM 트러블슈팅 경험']. "
            "'자료 근거 일치도', '구현 구체성', '구조 이해도' 같은 메타 카테고리는 절대 쓰지 말 것."
        ),
        min_length=1,
        max_length=3,
    )
...
## evaluation_targets 규칙

1. evaluation_targets는 이 질문이 실제로 파악하려는 포인트를 1~3개 한국어 구절로 작성합니다.
2. "자료 근거 일치도", "구현 구체성", "구조 이해도" 같은 메타 카테고리는 절대 출력하지 마세요. 이것들은 내부 채점 기준이며 사용자에게 노출하지 않습니다.
3. 대신 이 프로젝트의 어떤 의사결정·구현 디테일·지식 영역을 검증하려는지 구체적으로 적습니다.
   - 좋은 예: "DataLoader 배치 전략 이해", "체크포인트 복원 절차 경험", "OOM 트러블슈팅 경험", "토큰 갱신 정책의 trade-off 인식"
   - 나쁜 예: "구현 구체성", "구조 이해도", "트러블슈팅 경험" (메타 카테고리)
4. 항목은 질문이 다루는 핵심 주제를 그대로 명사구로 만든 형태로 작성합니다.
  ...
  "questions": [
    {
      ...
      "evaluation_targets": ["이 질문이 파악하려는 포인트 1~3개. 메타 카테고리 금지."],
      "source_refs": [
        {
...
2. 주어진 근거만 사용합니다.
   질문, 질문 의도, 기대 신호, 자료 발췌, 학생 답변 밖의 사실을 추측하지 않습니다.
...
JUDGE_SYSTEM = """당신은 제출 프로젝트 수행 진위 검증 검증의 1차 평가관입니다.
...
FOLLOW_UP_GENERATOR_SYSTEM = """당신은 제출 프로젝트 수행 진위 검증 검증의 꼬리질문 생성기입니다.
...
검증 질문, 답변, 루브릭 평가, source refs를 근거로 최종 판정과 프로젝트 영역별 분석을 작성합니다.
