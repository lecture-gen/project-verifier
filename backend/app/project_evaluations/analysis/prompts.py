...
3. needs_follow_up=true일 때 target_rubric_index에는 입력 채점 기준표 항목 중 **가장 부족한 한 개**의 index(0-based 정수)를 넣습니다. 여러 항목이 부족하더라도 이번 라운드에서는 하나만 지정합니다.
4. target_rubric_description에는 그 항목의 description을 입력 그대로 옮겨 적습니다.
5. request_to_generator에는 그 한 항목을 보충받기 위해 꼬리질문 생성기가 확인해야 할 포인트를 1~2문장으로만 적습니다. 두 항목을 동시에 요청하지 않습니다.
6. 누적된 꼬리질문 응답에서 그 항목이 이미 충분히 다뤄졌다면 다른 빈 항목 한 개를 지정합니다. 모든 항목이 채워졌다면 needs_follow_up=false 입니다.
7. needs_follow_up=false 이면 request_to_generator는 빈 문자열, target_rubric_index는 null, target_rubric_description은 빈 문자열이어야 합니다.
8. **루브릭 항목당 꼬리질문은 한 세션에서 최대 1회만 출제됩니다.** 입력의 `## 이미 꼬리질문이 출제된 채점 기준 항목 index` 목록에 있는 index는 다시 `target_rubric_index`로 선택하지 마세요. 학생이 그 항목에 답을 포기했거나(give-up) 충분히 답하지 못했더라도 같은 항목을 다시 묻지 않습니다.
9. 모든 채점 기준 항목이 이미 한 번씩 출제되어 미사용 항목이 없다면 `needs_follow_up=false`로 응답합니다. 이 경우 `target_rubric_index=null`, `target_rubric_description=""`, `request_to_generator=""` 입니다.
10. 미사용 항목이 여럿 남아 있다면 그중 누적 답변과 자료를 비교했을 때 **가장 부족한 항목 하나**를 선택합니다. 이미 출제된 항목은 후보에서 제외하고 비교합니다.
...
5. 원 질문이나 누적된 꼬리질문을 그대로 반복하지 않습니다. 학생이 이미 충분히 답한 부분은 다시 묻지 않습니다.
6. 이 응답은 한 라운드의 꼬리질문일 뿐이며, 이후 추가 라운드가 발생할 수 있습니다. 한 번에 모든 빈 항목을 다 묻지 말고, 평가관이 지정한 한 항목에만 집중합니다.
7. **학생이 무엇을 답해야 점수를 받는지 학생이 질문만 읽고도 알 수 있도록**, 답변의 형태를 가리키는 단서 한 가지를 질문 안에 자연스럽게 포함합니다.
   - 사용 가능한 단서 예: "본인이 직접 결정한 근거", "구체적인 한 가지 사례", "실제로 마주친 실패 케이스 한 가지", "실측한 수치나 관찰값", "선택하지 않은 대안과 그 이유", "어떤 트레이드오프를 받아들였는지" 등.
   - 단서는 질문 문장 안에 녹여서 작성합니다. "다음 형식으로 답하세요", "아래 양식대로", "1) ... 2) ..." 같은 메타 지시·양식 강요는 금지합니다.
8. **평가관이 지정한 채점 기준 항목(target_rubric_description)이 점수 부여 조건으로 명시하는 학생 행동을, 질문 표현이 직접 유도해야 합니다.**
   ...
   - 채점 기준이 "실패 케이스와 대응 설명"이면 질문도 "겪은 실패 한 가지와 어떻게 대응했는지"를 묻도록 표현하세요.
...
## 좋은 예 (학생이 답변 방향을 알 수 있는 단서 포함)

- "이 단계에서 동시성을 어떤 방식으로 직렬화했는지, 본인이 그 방식을 선택한 결정 근거 한 가지만 말해 줄 수 있을까요?"
- "이 처리 흐름에서 실제로 마주쳤던 실패 케이스 한 가지와 그것을 사용자에게 어떻게 다시 노출되도록 처리했는지 설명해 줄 수 있을까요?"
- "이 자료 인덱싱 방식을 선택하면서 받아들인 트레이드오프 한 가지를 구체적인 사례로 들어 설명해 줄 수 있을까요?"
    ...
    return [
        {"role": "system", "content": JUDGE_SYSTEM},
        {
            ...
            "content": (
                ...
                f"## 출제 의도\n{intent}\n\n"
                f"## 기대 답안\n{expected_answer}\n\n"
                f"## 채점 기준표 (각 항목 앞 [#index] 표기. needs_follow_up=true 면 가장 부족한 한 개의 index를 target_rubric_index에 넣고, 같은 항목의 description을 target_rubric_description에 그대로 옮길 것)\n{rubric_text}\n\n"
                ...
                "추가 확인이 필요하면 위 미사용 항목 중 가장 부족한 채점 기준 항목 한 개의 index를 target_rubric_index에 넣고, "
                "해당 항목 description을 target_rubric_description에, 확인 포인트를 request_to_generator에 1~2문장으로 적으세요."
            ),
        },
    ]
    ...
    return [
        {"role": "system", "content": FOLLOW_UP_GENERATOR_SYSTEM},
        {
            ...
            "content": (
                ...
                f"누적 꼬리질문 응답에서 이 항목이 일부 다뤄졌다면 비어 있는 부분만 한 문장으로 묻습니다.\n\n"
                f"## 원 질문\n{question}\n\n"
                f"## 출제 의도\n{intent}\n\n"
                f"## 기대 답안\n{expected_answer}\n\n"
                ...
                f"## 자료 발췌\n{snippets}\n\n"
                "위 한 항목을 보충받기 위한 단일 꼬리질문 한 문장만 생성하세요. "
                "한 질문에 두 가지를 결합하지 말고, 특정 파일/변수/식별자에 묶이지 말고, 120자 이내 한 문장으로 작성하세요."
            ),
        },
    ]
