...
학생가 제출한 프로젝트 zip 파일을 분석하고, 실시간 음성 검증로 수행 진위를 검증하는 시스템입니다.

## 기능 요약

- **zip 업로드**: PDF, PPTX, DOCX, README, 소스 코드가 포함된 단일 zip 파일 제출
- **자료 분석**: 기술 스택, 주요 기능, 아키텍처, 리스크 포인트 자동 추출
- **질문 생성**: Bloom's Taxonomy 기반 검증 질문 자동 생성
- **실시간 음성 검증**: OpenAI Realtime API를 이용한 양방향 음성 대화
...
4. "실시간 음성 검증 시작" 버튼 → 새 탭에서 음성 검증 진행
5. 검증 완료 후 "리포트 확인" 버튼 → 영역별 신뢰도 리포트 확인
```
